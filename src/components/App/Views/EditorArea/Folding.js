...
export function applyFoldedContentEdit(originalCode = '', projectedCode = '', lineItems = []) {
...
export function getExpandedFoldedSelection(originalCode, projectedCode, lineItems, start, end) {
  if (start === end) return '';
...
function getLineColumnAtIndex(value, index) {
  let line = 1;
  let lineStart = 0;
...
function getOriginalIndexForProjectedPosition(originalLines, lineItem, column) {
