...
export const isJavaScriptPath = (filePath = ''): boolean =>
  /\.(jsx?|tsx?|mjs|cjs)$/.test(filePath) || filePath === 'javascript' || filePath === 'typescript';

const isJsxPath = (filePath = '') => /\.(jsx|tsx)$/.test(filePath);

export function getJavaScriptBlockFolds(code = '', filePath = ''): CodeFold[] {
  if (!isJavaScriptPath(filePath) || !code) return [];

  const folds: CodeFold[] = [];
  const stack: Array<{ line: number; templateExpression?: boolean }> = [];
  const jsxStack: Array<{ line: number; name: string }> = [];
...
function getBlockStartLine(code: string, braceIndex: number, fallbackLine: number): number {
...
function findMatchingOpenParen(code: string, beforeIndex: number): number {
  let depth = 0;
...
function getLineForIndex(code: string, targetIndex: number): number {
  let line = 1;
...
function parseJsxTag(
  code: string,
  startIndex: number,
): { type: string; name: string; selfClosing: boolean; endIndex: number } | null {
...
function findLastMatchingJsxTag(
  stack: Array<{ line: number; name: string }>,
  name: string,
): number {
