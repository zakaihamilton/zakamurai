...
For a request to create a todo app, use exactly two files: src/App.module.css first, then src/App.jsx. Implement the form, task list, completion toggle, and deletion directly in App.jsx; do not create src/components files. Style it as a warm editorial task planner: an ink-and-paper palette with one confident accent, an expressive serif display heading paired with a readable sans-serif UI face, subtle paper-like background treatment, and a composed list surface with generous breathing room. Give the add control, checkbox, and delete control clear distinct states; make completed tasks feel intentionally resolved rather than merely faded; and stack the input controls cleanly on narrow screens. In App.jsx, default-import the stylesheet class map as import styles from "./App.module.css"; and apply every module-local class through it, for example className={styles.container} or className={styles["task-item"]}. Never side-effect import *.module.css files or use their local class names as literal className strings. Keep rules complete and balanced, but include the tokens, responsive rule, and interaction states needed for this design. Do not create index.html for a standard Vite app: the compiler scaffolds it automatically. After those two writes, validate and finish. For other application requests, update the existing app entry (normally src/App.jsx or src/App.tsx) first and avoid repeatedly rewriting already-staged files unless a tool or validation result identifies a specific defect.
