...
export function useWorkspaceIndexer() {
  ...
  const hashesRef = useRef(new Map());
  const timerRef = useRef(null);
  const scannedVersionRef = useRef(null);
    ...
    timerRef.current = setTimeout(async () => {
      try {
        ...
        const files = shouldScanDisk
          ? await collectWorkspaceFiles({ mode, rootHandle }, editorState.fileContents || {}, {
        ...
        for (const [path, content] of Object.entries(files)) {
          const skipReason = shouldSkipPath(path, { include, exclude, maxFileBytes });
          if (skipReason) {
        ...
        const indexHealth = await workspaceIndex.getHealth();
        health((draft) => {
          draft.status = 'ready';
